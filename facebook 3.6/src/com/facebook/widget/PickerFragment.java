...
public abstract class PickerFragment<T extends GraphObject> extends Fragment {
	...
	public void onInflate(Activity activity, AttributeSet attrs,
		...
		setShowPictures(a.getBoolean(
				R.styleable.com_facebook_picker_fragment_show_pictures,
				showPictures));
		String extraFieldsString = a
				.getString(R.styleable.com_facebook_picker_fragment_extra_fields);
		if (extraFieldsString != null) {
			String[] strings = extraFieldsString.split(",");
			setExtraFields(Arrays.asList(strings));
		}

		showTitleBar = a.getBoolean(
				R.styleable.com_facebook_picker_fragment_show_title_bar,
				showTitleBar);
		titleText = a
				.getString(R.styleable.com_facebook_picker_fragment_title_text);
		doneButtonText = a
				.getString(R.styleable.com_facebook_picker_fragment_done_button_text);
		titleBarBackground = a
				.getDrawable(R.styleable.com_facebook_picker_fragment_title_bar_background);
		doneButtonBackground = a
				.getDrawable(R.styleable.com_facebook_picker_fragment_done_button_background);
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		listView = (ListView) view
				.findViewById(R.id.com_facebook_picker_list_view);
		...
		listView.setOnLongClickListener(new View.OnLongClickListener() {
			...
			public boolean onLongClick(View v) {
				// We don't actually do anything differently on long-clicks, but
				// setting the listener
				// enables the selector transition that we have for visual
		...
		activityCircle = (ProgressBar) view
	...
	public void onActivityCreated(final Bundle savedInstanceState) {
		...
		sessionTracker = new SessionTracker(getActivity(),
				new Session.StatusCallback() {
					...
					public void call(Session session, SessionState state,
							Exception exception) {
						if (!session.isOpened()) {
							// When a session is closed, we want to clear out
							// our data so it is not visible to subsequent users
							clearResults();
						}
					}
				});
		...
		selectionStrategy.readSelectionFromBundle(savedInstanceState,
				SELECTION_BUNDLE_KEY);

		// Should we display a title bar? (We need to do this after we've
		// retrieved our bundle settings.)
		if (showTitleBar) {
			inflateTitleBar((ViewGroup) getView());
		}

		if (activityCircle != null && savedInstanceState != null) {
			boolean shown = savedInstanceState.getBoolean(
					ACTIVITY_CIRCLE_SHOW_KEY, false);
			if (shown) {
	 ...
	 * 
	 * @param onDoneButtonClickedListener
	 ...
	 * 
	 * @param session
	 *            the Session to use for Facebook requests, or null to use the
	 *            active session
	 */
	 ...
	 * 
	 * @param showTitleBar
	 *            true if a title bar should be displayed, false if not
	 */
	 ...
	 * 
	 * @param forceReload
	 *            if true, data will be loaded even if there is already data
	 *            being displayed (or loading); if false, data will not be
	 *            re-loaded if it is already displayed (or loading)
	 */
	 ...
	 * 
	 * @param inState
	 *            a Bundle containing keys corresponding to properties of the
	 *            PickerFragment
	 */
	...
	void saveSettingsToBundle(Bundle outState) {
		...
		if (!extraFields.isEmpty()) {
			outState.putString(EXTRA_FIELDS_BUNDLE_KEY,
					TextUtils.join(",", extraFields));
		}
	...
	void layoutActivityCircle() {
		// If we've got no data, make the activity circle full-opacity.
		// Otherwise we'll dim it to avoid
		// cluttering the UI.
	...
	void hideActivityCircle() {
		if (activityCircle != null) {
			// We use an animation to dim the activity circle; need to clear
			// this or it will remain visible.
	...
	private static void setAlpha(View view, float alpha) {
		// Set the alpha appropriately (setAlpha is API >= 11, this technique
	...
	private void setPickerFragmentSettingsFromBundle(Bundle inState) {
		// We do this in a separate non-overridable method so it is safe to call
		// from the constructor.
		if (inState != null) {
			showPictures = inState.getBoolean(SHOW_PICTURES_BUNDLE_KEY,
					showPictures);
			String extraFieldsString = inState
					.getString(EXTRA_FIELDS_BUNDLE_KEY);
			if (extraFieldsString != null) {
				String[] strings = extraFieldsString.split(",");
				setExtraFields(Arrays.asList(strings));
			}
			showTitleBar = inState.getBoolean(SHOW_TITLE_BAR_BUNDLE_KEY,
					showTitleBar);
			...
			String doneButtonTextString = inState
					.getString(DONE_BUTTON_TEXT_BUNDLE_KEY);
	...
	private void inflateTitleBar(ViewGroup view) {
		ViewStub stub = (ViewStub) view
				.findViewById(R.id.com_facebook_picker_title_bar_stub);
		if (stub != null) {
			...
			layoutParams.addRule(RelativeLayout.BELOW,
					R.id.com_facebook_picker_title_bar);
			...
			doneButton = (Button) view
					.findViewById(R.id.com_facebook_picker_done_button);
			if (doneButton != null) {
					...
					public void onClick(View v) {
						...
						if (onDoneButtonClickedListener != null) {
							onDoneButtonClickedListener
									.onDoneButtonClicked(PickerFragment.this);
						}
					}
				});
	...
	private void clearResults() {
		if (adapter != null) {
			...
			// Tell anyone who cares the data and selection has changed, if they
			// have.
			...
			if (wasSelection && onSelectionChangedListener != null) {
				onSelectionChangedListener
						.onSelectionChanged(PickerFragment.this);
			}
		}
	}
	...
	void updateAdapter(SimpleGraphObjectCursor<T> data) {
		if (adapter != null) {
			// As we fetch additional results and add them to the table, we do
			// not
			// want the items displayed jumping around seemingly at random,
			// frustrating the user's
			// attempts at scrolling, etc. Since results may be added anywhere
			// in
			// the table, we choose to try to keep the first visible row in a
			// fixed
			// position (from the user's perspective). We try to keep it
			// positioned at
			// the same offset from the top of the screen so adding new items
			// seems
			// smoother, as opposed to having it "snap" to a multiple of row
			// height

			// We use the second row, to give context above and below it and
			// avoid
			// cases where the first row is only barely visible, thus providing
			// little context.
			// The exception is where the very first row is visible, in which
			// case we use that.
			...
			GraphObjectAdapter.SectionAndItem<T> anchorItem = adapter
					.getSectionAndItem(anchorPosition);
			...
			if (view != null && anchorItem != null) {
				...
				final int newPositionOfItem = adapter.getPosition(
						anchorItem.sectionKey, anchorItem.graphObject);
	...
	abstract class LoadingStrategy {
		...
		public void attach(GraphObjectAdapter<T> adapter) {
			loader = (GraphObjectPagingLoader<T>) getLoaderManager()
					.initLoader(
							0,
							null,
							new LoaderManager.LoaderCallbacks<SimpleGraphObjectCursor<T>>() {
								...
								public void onLoaderReset(
									...
									LoadingStrategy.this
											.onLoadReset((GraphObjectPagingLoader<T>) loader);
								}
							});
			...
			loader.setOnErrorListener(new GraphObjectPagingLoader.OnErrorListener() {
				...
				public void onError(FacebookException error,
						GraphObjectPagingLoader<?> loader) {
					hideActivityCircle();
			...
			this.adapter
					.setOnErrorListener(new GraphObjectAdapter.OnErrorListener() {
						@Override
						public void onError(GraphObjectAdapter<?> adapter,
								FacebookException error) {
							if (onErrorListener != null) {
								onErrorListener.onError(PickerFragment.this,
										error);
							}
						}
					});
		...
		protected GraphObjectPagingLoader<T> onCreateLoader() {
			return new GraphObjectPagingLoader<T>(getActivity(),
					graphObjectClass);
		}

		protected void onStartLoading(GraphObjectPagingLoader<T> loader,
				Request request) {
			displayActivityCircle();
		}
	...
	abstract class PickerFragmentAdapter<U extends GraphObject> extends
			GraphObjectAdapter<T> {
		...
		void updateCheckboxState(CheckBox checkBox, boolean graphObjectSelected) {
			...
			int visible = (graphObjectSelected || selectionStrategy
					.shouldShowCheckBoxIfUnselected()) ? View.VISIBLE
					: View.GONE;
