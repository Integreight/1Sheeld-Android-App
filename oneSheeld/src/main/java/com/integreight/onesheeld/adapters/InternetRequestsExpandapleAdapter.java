...
public class InternetRequestsExpandapleAdapter extends BaseExpandableListAdapter {
    ...
    public View getGroupView(int groupPosition, boolean isExpanded,
        ...
        ((TextView) convertView
                .findViewById(R.id.requestID)).setText("Request : " + request.getId());
        ProgressBar prog =
        ...
        if (!request.isCancelled() || request.getStatus() == InternetRequest.REQUEST_STATUS.EXECUTED) {
            if (request.getStatus() == InternetRequest.REQUEST_STATUS.IN_QUEUE) {
                ...
                status.setBackgroundResource(R.drawable.internet_shield_red);
                ...
                prog.setVisibility(View.INVISIBLE);
                status.setBackgroundResource(request.getStatus() == InternetRequest.REQUEST_STATUS.EXECUTED && request.getResponse() != null && request.getResponse().getStatus() == InternetResponse.RESPONSE_STATUS.SUCCESSFUL ? R.drawable.internet_shield_red : R.drawable.internet_shield_red);
                status.setVisibility(View.VISIBLE);
            }
        } else {
            prog.setVisibility(View.INVISIBLE);
