...
public final class EditActivity extends AbstractPluginActivity
{
    ...
    protected void onCreate(final Bundle savedInstanceState)
    {
        ...
        ArrayAdapter<String> pinsArrayAdapter=new ArrayAdapter<String>(this,android.R.layout.simple_spinner_item, pins);
        ArrayAdapter<String> outputArrayAdapter=new ArrayAdapter<String>(this,android.R.layout.simple_spinner_item, output);
        ...
        if (null == savedInstanceState)
        {
            ...
            {
            	final int pin = localeBundle.getInt(PluginBundleManager.BUNDLE_EXTRA_PIN_NUMBER);
            	final boolean output = localeBundle.getBoolean(PluginBundleManager.BUNDLE_EXTRA_OUTPUT);
//                ((EditText) findViewById(android.R.id.text1)).setText(message);
            	pinsSpinner.setSelection(pin);
            	if(output)outputSpinner.setSelection(0);
            	else outputSpinner.setSelection(1);
            }
        }
    }
...
//            final String message = ((EditText) findViewById(android.R.id.text1)).getText().toString();
            final int pin=Integer.parseInt((pinsSpinner).getSelectedItem().toString());
            final boolean output=(outputSpinner).getSelectedItem().toString().toLowerCase().equals("high")?true:false;
//            if (message.length() > 0)
//            {
                ...
                final Bundle resultBundle =
                        PluginBundleManager.generateBundle(getApplicationContext(), pin,output);
                resultIntent.putExtra(com.twofortyfouram.locale.Intent.EXTRA_BUNDLE, resultBundle);

                /*
                 * The blurb is concise status text to be displayed in the host's UI.
                 */
                final String blurb = generateBlurb(getApplicationContext(), "Pin "+pin+" set to "+(output?"High":"Low"));
