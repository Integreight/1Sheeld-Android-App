...
public class QuickReturnHeaderHelper implements OnGlobalLayoutListener {
	...
	public QuickReturnHeaderHelper(Context context, int contentResId,
			int headerResId) {
	...
	public void setOnSnappedChangeListener(
			OnSnappedChangeListener onSnapListener) {
	...
	/**
	 * Invoked when the user stops scrolling the content. In response we might
	 * start an animation to leave the header in a fully open or fully closed
	 * state.
	 */
	private void onScrollIdle() {
		if (snapped) {
			// Only animate when header is out of its natural position (truly
			// over the content).
			return;
		}
	...
	private void onNewScroll(int delta) {
		...
		headerTop += delta;
		// I'm aware that offsetTopAndBottom is more efficient, but it gave me
